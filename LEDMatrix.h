...
public:
    ...
    /*
     * draw a number(digit) 
     * @param (x, y)  top left position, x has to be multiple of 8
     * @param number the number to be drawn
     */
    void drawDigits(uint16_t x, uint16_t y, uint8_t number);
    
    /*
     * draw a letter(or letters depending on font)
     * @param (x, y)  top left position, x has to be multiple of 8
     * @param letterIndex the letter(s) to be drawn
     */
    void drawLetters(uint16_t x, uint16_t y, uint8_t letterIndex);
