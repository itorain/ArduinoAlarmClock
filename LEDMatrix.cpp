...
void drawDigits(uint16_t x, uint16_t y, uint8_t n)
{
    if ((n >= 10) || (0 != (x % 8))) {
        return;
    }
    ...
    const uint8_t *pSrc = digits + n * 16;
...
void drawLetters(uint16_t x, uint16_t y, uint8_t n)
{
    if ((n >= 14) || (0 != (x % 8))) {
        return;
    }
    ...
    uint8_t *pDst = displaybuf + y * (WIDTH / 8) + x / 8;
    const uint8_t *pSrc = letters + n * 16;
    for (uint8_t i = 0; i < 16; i++) {
        *pDst = *pSrc;
        pDst += WIDTH / 8;
        pSrc++;
